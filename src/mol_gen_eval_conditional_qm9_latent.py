...
from src.models.components import centralize
...
class TestDataLoader:
    def __init__(
        ...
        self.num_samples = data.num_graphs
        self.i = 0
    ...
    def sample(self) -> Dict[str, Any]:
        ...
        for i in range(batch.num_graphs):
            # 计算当前图的节点数量
            num_node = (batch.batch == i).sum().item()  # 统计 batch 中等于 i 的节点数量
            num_nodes.append(num_node)
        num_nodes=torch.tensor(num_nodes).to(self.device)
        batch.props_context = qm9utils.prepare_context(
            list(self.cfg.model.module_cfg.conditioning),
            batch,
            self.props_norms
        ).type(torch.float32)
        context = batch.props_context
        x = batch.x
        ...
        elements,indices=torch.unique(batch.batch,return_inverse=True)
        first_indices=torch.full((batch.num_graphs,),-1,dtype=torch.int64)
        for i in range(len(batch.batch)):
            ele=batch.batch[i].item()
            if first_indices[ele]==-1:
                first_indices[ele]=i
        select_contex=context[first_indices]
        # build context
        prop_key = self.props_distr.properties[0]
        if self.unknown_labels:
            context[:] = self.props_distr.normalizer[prop_key]["mean"]
            select_contex[:] = self.props_distr.normalizer[prop_key]["mean"]
        else:
            context = (
                context * self.props_distr.normalizer[prop_key]["mad"] + self.props_distr.normalizer[prop_key]["mean"]
            )
            select_contex = (
                    select_contex * self.props_distr.normalizer[prop_key]["mad"] + self.props_distr.normalizer[prop_key][
                "mean"]
            )
...
def evaluate(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    device = (
        ("cuda:1" if isinstance(cfg.trainer.devices, int) else f"cuda:{cfg.trainer.devices[0]}")
...
@hydra.main(version_base="1.2", config_path= "../configs", config_name="mol_gen_eval_conditional_qm9_latent.yaml")
