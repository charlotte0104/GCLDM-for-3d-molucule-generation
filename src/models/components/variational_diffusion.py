...
class EquivariantVariationalDiffusion(nn.Module):
    ...
    def mol_gen_sample(
        ...
        # for s in tqdm(reversed(range(0, 20))):
            # print(s,end='')
...
class GCPNetAutoEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.num_node_scalar_features = dataloader_cfg.num_atom_types + dataloader_cfg.include_charges
        self.T = diffusion_cfg.num_timesteps
        self.encoder=GCPNetEncoder(
            model_cfg=model_cfg,
            module_cfg=module_cfg,
            layer_cfg=layer_cfg,
            diffusion_cfg=diffusion_cfg,
            dataloader_cfg=dataloader_cfg)
        self.decoder=GCPNetDecoder(
            model_cfg=model_cfg,
            module_cfg=module_cfg,
            layer_cfg=layer_cfg,
            diffusion_cfg=diffusion_cfg,
        ...
        self.encoded_h_dim=self.encoder.encoded_h_dim
        # Forward pass #
    ...
    def sample_combined_position_feature_noise(
        ...
        if generate_x_only:
            ...
            return z_x
        z_h = self.sample_gaussian_with_mask(
            size=(len(batch_index), self.encoded_h_dim),
            node_mask=node_mask,
            device=batch_index.device
        )
        z = torch.cat([z_x, z_h], dim=-1)
    ...
    #encode 直接改变batch的内容，而decode返回解码后的x h
    ...
    def decode(self,batch):
        ...
        h_int=h_recon[:,-1:]
        h_cat=h_recon[:,:-1]
        num_classes=h_cat.size[-1]
        h_int=(torch.round(h_int)*batch.mask.float().unsqueeze(-1)).squeeze(-1)
        h_cat=F.one_hot(torch.argmax(h_cat,dim=-1),num_classes)*batch.mask.float().unsqueeze(-1)
        h_recon={"categorical": h_cat, "integer": h_int}

        return x_recon,h_recon
    def decode_without_roundint_and_onehot(self,batch):
        xh = torch.cat([batch.x, batch.h], dim=-1)
        batch,xh_recon=self.decoder(batch,xh)
        x_recon=xh_recon[:, :self.num_x_dims]
        h_recon=xh_recon[:, self.num_x_dims:]
        if self.include_charges:
            h_int=h_recon[:,-1:]
            h_cat=h_recon[:,:-1]
            h_int=(h_int*batch.mask.float().unsqueeze(-1)).squeeze(-1)
            h_cat=h_cat*batch.mask.float().unsqueeze(-1)
        else:
            h_cat=h_recon
            h_int=torch.zeros(0,device=x_recon.device)
        h_recon={"categorical": h_cat, "integer": h_int}

        return x_recon,h_recon
    def decode_xh(self,batch,xh):
        batch, xh_recon = self.decoder(batch, xh)
        x_recon = xh_recon[:, :self.num_x_dims]
        h_recon = xh_recon[:, self.num_x_dims:]
        h_int = h_recon[:, -1:]
        h_cat = h_recon[:, :-1] * batch.mask
        h_int = torch.round(h_int) * batch.mask
        h_recon = {"categorical": h_cat, "integer": h_int}

        return x_recon, h_recon


    def compute_reconstruction_error(self,xs,xr,hs,hr,idx,mask,num_nodes,batch_size):
        error_x=self.sum_node_features_except_batch((xs-xr)**2,idx)
        # error_x = self.sum_node_features_except_batch((xs - xr) ** 2, idx)
        cat=hs["categorical"]
        cat_recon=hr["categorical"]
        error_cat=((cat-cat_recon)**2).sum(-1)
        error_cat=F.cross_entropy(cat_recon,cat.argmax(dim=-1),reduction='none')
        error_cat=self.sum_node_features_except_batch(error_cat.unsqueeze(-1),idx)
        if self.include_charges:
            int=hs["integer"]
            int_recon=hr["integer"]
            error_int=self.sum_node_features_except_batch((int-int_recon).unsqueeze(-1)**2,idx)
        else:
            error_int=0

        error=error_int+error_cat


        denom=(self.num_atom_types+self.include_charges+self.num_x_dims)*(num_nodes/batch_size)
        error=error/denom
        return error
    def forward(self,batch):
        ...
        x_recon, h_recon = self.decode_without_roundint_and_onehot(batch)
        error_recon = self.compute_reconstruction_error(x_saved, x_recon, h_saved, h_recon, batch.batch, batch.mask,
                                                            num_nodes=batch.num_nodes, batch_size=batch.batch_size)
        return error_recon
    ...
    def test(self,batch):
        ...
        x_recon, h_recon = self.decode_without_roundint_and_onehot(batch)
        if self.include_charges:
            xh_recon = torch.cat([x_recon, h_recon["categorical"], h_recon["integer"].unsqueeze(-1)], dim=-1)
        else:
            xh_recon = torch.cat([x_recon, h_recon["categorical"]], dim=-1)
        # x_ = xh[:, :self.num_x_dims].detach().cpu()
        atom_types_recon = (
            xh_recon[:, self.num_x_dims:-1].argmax(-1).detach()
            if self.include_charges
            else xh_recon[:, self.num_x_dims:].argmax(-1).detach()
        )
        atom_types_saved=(
            xh[:, self.num_x_dims:-1].argmax(-1).detach()
            if self.include_charges
            else xh_recon[:, self.num_x_dims:].argmax(-1).detach()
        )
        equal_elements = torch.sum(atom_types_recon == atom_types_saved).item()
        all_elements=atom_types_recon.size(0)

        return equal_elements,all_elements