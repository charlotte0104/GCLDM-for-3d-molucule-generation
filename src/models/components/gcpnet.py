...
class GCPNetDynamics(nn.Module):
    def __init__(
        ...
        if diffusion_cfg.if_latent_diffusion :
            h_input_dim_= model_cfg.auto_encoder_h_out_dim
        else:
            h_input_dim_ = (
        ...
        num_interaction_layers =model_cfg.latent_diffusion_gcp_layers if diffusion_cfg.if_latent_diffusion else model_cfg.num_encoder_layers
...
class GCPNetEncoder(nn.Module):
    def __init__(
        ...
        #h_input_conditioning_dim = int(diffusion_cfg.condition_on_time)  # time-conditioning
        h_input_conditioning_dim = len(module_cfg.conditioning)  # context-conditioning
        final_h_out_dim=model_cfg.auto_encoder_h_out_dim
        self.encoded_h_dim=final_h_out_dim
        # input feature dimensionalities while considering self-conditioning
        h_input_dim = (
            h_input_dim_ * 2
            if diffusion_cfg.self_condition and diffusion_cfg.diffusion_target in NODE_FEATURE_DIFFUSION_TARGETS
        ...
        self.final_mlp=nn.Sequential(
            nn.Linear(h_input_dim,256),
            torch.nn.SiLU(),
            nn.Linear(256,final_h_out_dim)
        )
        self.edge_input_dims = ScalarVector(e_input_dim, xi_input_dim)
        self.node_input_dims = ScalarVector(h_input_dim + h_input_conditioning_dim, chi_input_dim)
        self.edge_dims = ScalarVector(model_cfg.e_hidden_dim, model_cfg.xi_hidden_dim)
        ...
        self.interaction_layers = nn.ModuleList(
            ...
            ) for _ in range(model_cfg.auto_encoder_gcp_layers)
        )
    ...
    def forward(
        ...
        **kwargs: Any
        ...
        Batch,
        TensorType["batch_num_nodes", "num_x_dims_plus_final_h_out_dim"]
    ]:
        return self.atom_types_and_coords_forward(batch, xh,  **kwargs)
    ...
    def atom_types_and_coords_forward(
        ...
        **kwargs: Any
        ...
        # update graph features using a series of geometric message-passing layers
        for layer in self.interaction_layers:
            (h, chi) = layer(                                              #(h, chi), batch.x = layer(
                (h, chi),
                (e, xi),
                batch.edge_index,
                batch.f_ij,
        ...
        vel = batch.x  * batch.mask.float().unsqueeze(-1)  # use delta(x) to estimate noise
        h_final = batch.h
        ...
        # detect and nullify any invalid node position predictions
        # if vel.isnan().any():
        #     log.warning(f"Detected NaN in `vel` -> resetting GCPNetEncoder `vel` .")
        #     vel = torch.zeros_like(vel)
...
"""
对于decoder，我们希望传入batch后，对xh解码，但是不改变batch.x，batch.h
而是只返回解码后的xh
因为在解码，并求得重建损失之后，latent_xh还需要用到
"""
class GCPNetDecoder(nn.Module):
    def __init__(
        ...
        # feature dimensionalities
        h_input_dim_ = model_cfg.auto_decoder_h_in_dim
        #h_input_conditioning_dim = int(diffusion_cfg.condition_on_time)  # time-conditioning
        h_input_conditioning_dim = len(module_cfg.conditioning)  # context-conditioning
        final_h_out_dim=dataloader_cfg.num_atom_types + dataloader_cfg.include_charges
        self.final_h_out_dim=final_h_out_dim
        # input feature dimensionalities while considering self-conditioning
        h_input_dim = (
            h_input_dim_ * 2
            if diffusion_cfg.self_condition and diffusion_cfg.diffusion_target in NODE_FEATURE_DIFFUSION_TARGETS
        ...
        self.interaction_layers = nn.ModuleList(
            GCPInteractions(
                ...
                # update_node_positions=False
                update_node_positions=False
            ) for _ in range(model_cfg.auto_decoder_gcp_layers)
        )
        self.final_mlp=nn.Sequential(
            torch.nn.Linear(self.final_h_out_dim,256),
            torch.nn.Linear(256,256),
            torch.nn.Linear(256, 256),
            torch.nn.Linear(256, 256),
            torch.nn.Linear(256,self.final_h_out_dim),
        ...
        if diffusion_cfg.diffusion_target in NODE_FEATURE_DIFFUSION_TARGETS:
            ...
            h_input_dim_without_self_conditioning = h_input_dim_ + h_input_conditioning_dim
            self.scalar_node_projection_gcp = module_cfg.selected_GCP(
                self.node_dims, (final_h_out_dim, 0),
                nonlinearities=(None, None),
                scalar_gate=module_cfg.scalar_gate,
                vector_gate=module_cfg.vector_gate,
                frame_gate=module_cfg.frame_gate,
    ...
    def forward(
        self,
        batch: Batch,
        xh: TensorType["batch_num_nodes", "nnum_x_dims_plus_final_h_out_dim"],
        **kwargs: Any
    ) -> Tuple[
        Batch,
        TensorType["batch_num_nodes", "num_x_dims_plus_num_node_scalar_features"]
    ...
    def atom_types_and_coords_forward(
        self,
        batch: Batch,
        xh: TensorType["batch_num_nodes", "num_x_dims_plus_final_h_out_dim"],
        xh_self_cond: Optional[TensorType["batch_num_nodes", "num_x_dims_plus_num_node_scalar_features"]] = None,
        **kwargs: Any
    ) -> Tuple[
        Batch,
        ...
        # detect and nullify any invalid node position predictions
        if vel.isnan().any():
            log.warning(f"Detected NaN in `vel` -> resetting GCPNetD `vel`.")
            vel = torch.zeros_like(vel)
        if h.isnan().any():
            log.warning(f"Detected NaN in `vel` -> resetting GCPNetD `vel`.")
        if chi.isnan().any():
...
class GCPUInteractions(nn.Module):
    def __init__(
            self,
            in_node_dims: ScalarVector,
            out_node_dims: ScalarVector,
            edge_dims: ScalarVector,
            cfg: DictConfig,
            layer_cfg: DictConfig,
            dropout: float = 0.0,
        ...
        # PyTorch modules #
        self.res_mlp=torch.nn.Linear(in_features=in_node_dims.scalar+out_node_dims.scalar,out_features=out_node_dims.scalar)
        # geometry-complete message-passing neural network
        message_function = GCPMessagePassing
        ...
        self.gcp_norm = nn.ModuleList([GCPLayerNorm(in_node_dims, use_gcp_norm=layer_cfg.use_gcp_norm)])
        self.gcp_dropout = nn.ModuleList([GCPDropout(dropout, use_gcp_dropout=layer_cfg.use_gcp_dropout)])

        # build out feedforward (FF) network modules
        hidden_dims = (
            (out_node_dims.scalar, out_node_dims.vector)
            if layer_cfg.num_feedforward_layers == 1
            else (4 * out_node_dims.scalar, 2 * out_node_dims.vector)
        )
        ff_interaction_layers = [
            ff_without_res_GCP(
                (in_node_dims.scalar +out_node_dims.scalar, in_node_dims.vector + out_node_dims.vector), hidden_dims,
                nonlinearities=(None, None) if layer_cfg.num_feedforward_layers == 1 else cfg.nonlinearities,
                feedforward_out=layer_cfg.num_feedforward_layers == 1
            )
        ]
        ...
        if update_node_positions:
            ...
            position_output_dims = (
                out_node_dims
                if getattr(cfg, "update_positions_with_vector_sum", False)
                else (out_node_dims.scalar, 1)
            )
            self.node_position_update_gcp = ff_without_res_GCP(
                out_node_dims, position_output_dims,
                nonlinearities=cfg.nonlinearities
            )
    ...
    def forward(
        ...
        hidden_residual=self.gcp_dropout[0](hidden_residual)
        scalar=self.res_mlp(torch.concat((node_rep.scalar,hidden_residual.scalar),dim=-1))
        vector=node_rep.vector+hidden_residual.vector
        node_rep = ScalarVector(scalar, vector)
        # apply GCP normalization (2)
        if not self.pre_norm:
            node_rep = self.gcp_norm[0](node_rep)
        ...
        return node_rep, node_pos
class GCPUNetDynamics(nn.Module):
    def __init__(
            self,
            model_cfg: DictConfig,
            module_cfg: DictConfig,
        ...
        h_hidden=model_cfg.h_hidden_dim
        self.h_unet_dims=[h_hidden,h_hidden//2,h_hidden//4,h_hidden//8,h_hidden//16,h_hidden//16,h_hidden//8,h_hidden//4,h_hidden//2,h_hidden]
        self.edge_input_dims = ScalarVector(e_input_dim, xi_input_dim)
        self.node_input_dims = ScalarVector(h_input_dim + h_input_conditioning_dim, chi_input_dim)
        self.edge_dims = ScalarVector(model_cfg.e_hidden_dim, model_cfg.xi_hidden_dim)
        #self.node_dims = ScalarVector(model_cfg.h_hidden_dim, model_cfg.chi_hidden_dim)
        self.node_dims=[ScalarVector(h_dims,model_cfg.chi_hidden_dim) for h_dims in self.h_unet_dims]
        self.num_context_node_features = len(module_cfg.conditioning)
        ...
        self.down_sampling=nn.ModuleList(
            GCPUInteractions(
                ...
                dropout=model_cfg.dropout,
                update_node_positions=True
            ) for i in range((num_interaction_layers-1)//2)
        )
        self.mid_layer=GCPUInteractions(
            self.node_dims[(num_interaction_layers-1)//2],
            self.node_dims[(num_interaction_layers-1)//2+1],
            self.edge_dims,
            cfg=module_cfg,
            layer_cfg=layer_cfg,
            dropout=model_cfg.dropout,
        ...
        self.up_sampling=nn.ModuleList(
            GCPUInteractions(
                ...
                dropout=model_cfg.dropout,
                update_node_positions=True
            ) for i in range((num_interaction_layers-1)//2+1,num_interaction_layers)
        )
        if diffusion_cfg.diffusion_target in NODE_FEATURE_DIFFUSION_TARGETS:
            # scalar node projection via GCP
            h_input_dim_without_self_conditioning = h_input_dim_ + h_input_conditioning_dim
            self.scalar_node_projection_gcp = module_cfg.selected_GCP(
                self.node_dims[0], (h_input_dim_without_self_conditioning, 0),
                nonlinearities=(None, None),
                scalar_gate=module_cfg.scalar_gate,
                vector_gate=module_cfg.vector_gate,
                frame_gate=module_cfg.frame_gate,
    ...
    def atom_types_and_coords_forward(
        ...
        i=((self.num_interaction_layers-1)//2)-1
