...
def compute_context(dataloaders,model_config,dataloader_cfg):
    hparams=model_config
    if len(model_config.module_cfg.conditioning) > 0 :
        props_norms = compute_mean_mad(
            dataloaders, list(hparams.module_cfg.conditioning), dataloader_cfg.dataset
        )
        props_distr = PropertiesDistribution(
            dataloaders["train"], hparams.module_cfg.conditioning, device=device
        )
        ...
        dummy_props_context = qm9utils.prepare_context(
            list(hparams.module_cfg.conditioning),
            dummy_data,
            props_norms
        ).type(torch.float32)
        num_context_node_feats = dummy_props_context.shape[-1]
        context_info = {
...
def save_checkpoint(model,optimizer,epoch,best_flag=False,mini_batch_flag=False):
    ...
    if best_flag==False:
        file_name="/tmp/pycharm_project_80/checkpoint_latent_condition_qm9/homo/"+"{}_epoch.pth".format(epoch)
    else:
        if mini_batch_flag == True :
            file_name="/tmp/pycharm_project_80/checkpoint_latent_condition_qm9/homo/"+"best_loss_mini_batch.pth"
        else :
            file_name = "/tmp/pycharm_project_80/checkpoint_latent_condition_qm9/homo/" + "best_loss.pth"
...
def train(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    if len(cfg.model.module_cfg.conditioning)>0 :
        context_info=compute_context(dataloaders,cfg.model,cfg.datamodule.dataloader_cfg)
    ...
    model.to(device)
    # vae_para_path="/tmp/pycharm_project_763/checkpoint_gcp_autoencoder/"+"19_epoch.pth"
    # vae_trained_params = torch.load(vae_para_path)
    # model.ddpm.vae.load_state_dict(vae_trained_params['model_state_dict'])
    model_para_path="/tmp/pycharm_project_80/checkpoint_latent_condition_qm9/homo/"+"best_loss_mini_batch.pth"
    ...
    loss_best = 1000000
    for epoch in range(epoch_now,2000):
        batch_idx=0
